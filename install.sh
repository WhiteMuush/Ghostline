...
print_summary() {
    ...
    printf "  [5]  CrackMapExec     -> %s\n" "$(command -v crackmapexec 2>/dev/null || command -v cme 2>/dev/null || echo '~/.local/bin/')"
    printf "  [6]  adidnsdump       -> %s\n" "$(command -v adidnsdump 2>/dev/null || echo '~/.local/bin/')"
    printf "  [7]  BloodHound.py    -> %s\n" "$(command -v bloodhound-python 2>/dev/null || echo '~/.local/bin/')"
    printf "  [8]  ridenum          -> %s\n" "/usr/local/bin/ridenum"
    printf "  [9]  Impacket         -> %s\n" "$(command -v secretsdump.py 2>/dev/null || command -v impacket-secretsdump 2>/dev/null || echo '~/.local/bin/')"
    printf "  [10] dnsrecon/dnsenum -> %s\n" "$(command -v dnsrecon 2>/dev/null || echo 'not found')"
    printf "  [11] Kerbrute         -> %s\n" "/usr/local/bin/kerbrute"
    printf "  [12] ldapdomaindump   -> %s\n" "$(command -v ldapdomaindump 2>/dev/null || echo '~/.local/bin/')"
