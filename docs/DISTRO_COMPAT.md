...
The tools Ghostline wraps (crackmapexec, responder, impacket, metasploit and
...
- **Any other distro** (Fedora, Arch, openSUSE, Alpine, atomic distros like
  Bazzite or Silverblue) is offered a container: Ghostline runs inside one
...
unprivileged TCP connect scan. For raw-socket scanning (nmap SYN, OS
detection), run the toolkit natively on a Debian/Kali host or in a rootful
container.
